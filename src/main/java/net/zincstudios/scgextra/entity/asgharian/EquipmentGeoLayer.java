 ...
 * @param <T>
 */
public class EquipmentGeoLayer<T extends Mob & GeoAnimatable> extends BlockAndItemGeoLayer<T> {

    private GeoBone cachedBone;
    private final float gunTilt;

    public EquipmentGeoLayer(GeoRenderer<T> renderer, float gunTilt) {
        super(renderer);
        this.gunTilt = gunTilt;
    }

    public EquipmentGeoLayer(GeoRenderer<T> renderer) {
        this(renderer, 0);
    }
