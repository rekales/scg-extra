...
public class CandleFiendEntity extends Monster implements GeoEntity {
    ...
    public enum BehaviorState {
        NONE, ENRAGED, SLASHING, SLAM, DYING
    }
    ...
    public void tick() {
        ...
        if (this.getBehaviourState() == BehaviorState.ENRAGED) {
            this.getNavigation().stop();
            if (ENRAGE_DURATION_TICKS - 1 == this.enragedTicks) {
        ...
        if (!this.level().isClientSide) {
            if (this.getTarget() != null) {
                if (this.lastTarget == null) {
                    this.enragedTicks = ENRAGE_DURATION_TICKS;  // Match with animation
                }
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        ...
        controllers.add(new ExpandedAnimationController<>(this, "behaviour", 3, state -> PlayState.STOP)
